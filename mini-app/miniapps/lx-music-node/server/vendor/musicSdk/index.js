// 移植自 lxserver src/modules/utils/musicSdk/index.js
// 转换为 CommonJS

const kw = require('./kw/index')
const kg = require('./kg/index')
const tx = require('./tx/index')
const wy = require('./wy/index')
const mg = require('./mg/index')
const bd = require('./bd/index')
const xm = require('./xm.js')
const { supportQuality } = require('./api-source')
...
module.exports = {
  ...
  async searchMusic({ name, singer, source: s, limit = 25, page = 1 }) {
    ...
    for (const source of sources.sources) {
      ...
      tasks.push(
        sources[source.id].musicSearch.search(`${musicName} ${singer || ''}`.trim(), page, limit)
          .catch(err => { console.error(`[MusicSdk] Search error in ${source.id}:`, err); return null })
      )
    }
  ...
  async findMusic({ name, singer, albumName, interval, source: s }) {
    ...
    const trimStr = str => typeof str === 'string' ? str.trim() : (str || '')
    const filterStr = str => typeof str === 'string'
      ? str.replace(/\s|'|\\. |,|，|&|"|、|\\(|\\)|（|）|`|~|-|<|>|\\||\\/|\\]|\\[|!|！/g, '')
      : String(str || '')
